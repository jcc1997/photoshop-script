...
const MissingAfterEffects = 'Photoshop could not be found.';
...
function find_application_dir(applications_dir) {
  ...
  for (let i = 0; i < files_in_applications_dir.length; i++) {
    ...
    //If some wise-ass named a folder 'Adobe Photoshop Plugins' or whatever and leaves
    //it in his applications directory, we don't want to choose it. We want to choose an application_dir
    //that's going to have an Photoshop.app in it.
...
function find_application(application_dir) {
  ...
  for (let i = 0; i < files_in_application_dir.length; i++) {
    ...
    if (file.includes('Adobe Photoshop') && file.endsWith('.app'))
...
function check_for_missing_app_hack(error) {
  ...
  //DoScript couldn't complete, which means the suggested Photoshop app doesnt exist
...
function create_apple_script(command, sync) {
  const apple_script_file = path.join(os.tmpdir(),`ps-command-${uuid.v4()}.scpt`);
  ...
  do javascript "${esc(command.toString(), { quotes: 'double'})}"
end tell`;

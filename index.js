...
const Errors = {
  UnsupportedPlatform : 'Cannot run Photoshop commands in an environment it can\'t be installed in.',
  BadExecuteArgument : 'execute expects a function or AfterEffectsCommand instance.',
  ApplicationNotFound : 'Cannot execute command, Photoshop could not be found in your application directory. Install Photoshop in your application directory, or provide a path in program option.',
...
function create_result_file_name(command) {
  command.result_file = `ps-result-${uuid.v4()}.js`;
}
...
function get_results(command) {
  ...
  try {
    ...
    // Photoshop seems don't have a 'TemporaryItems' subfolder.
